...
**Core principle:** Verify tests → Manual check → Tidy history → Size report → Detect environment → Present options → Execute choice → Compound step → Clean up.
...
to describe the change, not the process. Keep task boundaries as commits.
Never rewrite commits already on a shared branch. If the branch is already
pushed, ask before rewriting.
...
Report files and lines changed.
Above 500 lines or 10 files, offer a split into stacked PRs by task. The human
decides.
